<template>
  ...
  <span class="info-value">{{ request.userName || request.userId }}</span>
</div>
              <div class="info-row">
              ...
              <div class="info-block" v-if="request.selectedProducts?.length">
                <span class="info-label">
                  Sản phẩm đính kèm
                  <span class="info-label-count">({{ request.selectedProducts.length }})</span>
                </span>
                <div class="product-list">
                  <div v-for="(p, i) in request.selectedProducts" :key="i" class="product-item">
                    ...
                    <span class="product-id-badge" v-if="p.productId">
                      ID: {{ shortId(p.productId) }}
                    </span>
                  </div>
                </div>
              </div>
...
.hero-mesh { position: absolute; inset: 0; background: radial-gradient(ellipse 80% 60% at 20% 0%, rgba(37,99,235,.35), transparent), radial-gradient(ellipse 60% 50% at 80% 100%, rgba(124,58,237,.3), transparent), radial-gradient(ellipse 50% 40% at 50% 50%, rgba(16,185,129,.08), transparent); }
...
.main-panel { max-width: 1100px; margin: -32px auto 0; padding: 0 24px 60px; position: relative; z-index: 10; }
...
.info-card, .update-card, .messages-card { background: white; border-radius: 22px; padding: 24px 26px; border: 1.5px solid #e8edf8; box-shadow: 0 4px 24px rgba(37,99,235,.06); }
...
.info-row { display: flex; align-items: center; justify-content: space-between; padding: 10px 0; border-bottom: 1px solid #f8faff; }
.info-label { font-size: .76rem; font-weight: 700; color: #4f46e5; text-transform: uppercase; letter-spacing: .04em; }
.info-label-count { font-size: .7rem; color: #94a3b8; font-weight: 500; text-transform: none; letter-spacing: 0; margin-left: 4px; }
...
.info-admin-note { margin: 8px 0 0; font-size: .85rem; color: #713f12; line-height: 1.6; background: #fef9c3; border-left: 3px solid #facc15; padding: 10px 14px; border-radius: 10px; }

/* ══ PRODUCT LIST ══ */
.product-list { display: flex; flex-direction: column; gap: 8px; margin-top: 10px; }
.product-item { display: flex; align-items: center; gap: 10px; padding: 10px 12px; border-radius: 12px; border: 1.5px solid #e0e7ff; background: #f8faff; }
.product-img-wrap { width: 48px; height: 48px; border-radius: 8px; overflow: hidden; border: 1px solid #e0e7ff; flex-shrink: 0; background: #f1f5f9; }
...
.product-img-placeholder svg { width: 18px; height: 18px; }
.product-info { flex: 1; min-width: 0; }
.product-name { font-size: .84rem; font-weight: 700; color: #0f172a; margin: 0 0 2px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.product-meta { font-size: .72rem; color: #64748b; margin: 0 0 3px; }
.product-price-row { display: flex; align-items: center; gap: 6px; }
.product-qty { font-size: .72rem; color: #94a3b8; font-weight: 600; }
.product-price { font-size: .78rem; font-weight: 700; color: #e11d48; }
.product-id-badge { flex-shrink: 0; font-size: .68rem; font-family: monospace; color: #64748b; background: #f1f5f9; border: 1px solid #e2e8f0; padding: 2px 6px; border-radius: 6px; }

.info-images { display: flex; flex-wrap: wrap; gap: 10px; margin-top: 16px; }
.info-img-wrap { width: 70px; height: 70px; border-radius: 12px; overflow: hidden; border: 1.5px solid #e0e7ff; cursor: zoom-in; transition: transform .2s; }
...
.form-select, .form-textarea { border: 1.5px solid #e0e7ff; border-radius: 11px; padding: 10px 13px; font-size: .85rem; outline: none; width: 100%; box-sizing: border-box; transition: border-color .2s, box-shadow .2s; font-family: inherit; color: #0f172a; background: white; }
.form-select:focus, .form-textarea:focus { border-color: #a5b4fc; box-shadow: 0 0 0 4px rgba(37,99,235,.08); }
.form-textarea { resize: vertical; }

.msg { display: flex; align-items: center; gap: 7px; padding: 10px 14px; border-radius: 11px; font-size: .8rem; font-weight: 600; margin: 0 0 12px; }
...
.btn-save { width: 100%; padding: 12px; border-radius: 12px; font-size: .86rem; font-weight: 800; color: white; background: linear-gradient(135deg, #2563eb, #4f46e5); border: none; cursor: pointer; position: relative; overflow: hidden; box-shadow: 0 5px 16px rgba(37,99,235,.3); transition: transform .2s, box-shadow .2s; }
...
.btn-shine { position: absolute; top: 0; left: -80%; width: 60%; height: 100%; background: linear-gradient(90deg, transparent, rgba(255,255,255,.25), transparent); transform: skewX(-20deg); transition: left .5s; }
.btn-save:hover:not(:disabled) .btn-shine { left: 130%; }
.loading-wrap { display: inline-flex; align-items: center; gap: 7px; }
.spinner-sm { width: 13px; height: 13px; border-radius: 50%; border: 2px solid rgba(255,255,255,.4); border-top-color: white; animation: spin .7s linear infinite; }
...
.chat-messages { display: flex; flex-direction: column; gap: 10px; max-height: 420px; overflow-y: auto; padding-right: 4px; margin-bottom: 16px; flex: 1; }
.chat-empty { text-align: center; color: #94a3b8; font-size: .85rem; padding: 24px 0; }
.msg-bubble { display: flex; flex-direction: column; gap: 3px; max-width: 82%; padding: 10px 14px; border-radius: 14px; }
