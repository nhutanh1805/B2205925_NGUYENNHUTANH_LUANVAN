<template>
  <div class="ts-root">
    ...
    <!-- ADMIN BADGE (fixed, always visible) -->
          ...
          >
            <router-link :to="`/products/${product._id}`">
              ...
              <div class="ts-flash-info">
                ...
                <div class="ts-flash-admin-actions">
                  <router-link :to="`/products/edit/${product._id}`" class="ts-btn-cart">Sửa sản phẩm ✏️</router-link>
                </div>
              </div>
            </router-link>
          </div>
    ...
    <section class="ts-reviews">
      <div class="ts-container">
        <div class="ts-section-header text-center">
          ...
          <p class="ts-section-sub">Theo dõi phản hồi mới nhất từ người mua hàng</p>
        </div>
        ...
        <div class="text-center mt-5">
          <router-link to="/reviews" class="ts-btn ts-btn--outline">
            Quản lý đánh giá →
          </router-link>
        </div>
      </div>
            ...
            <router-link :to="`/products/${product._id}`">
              <div class="ts-arrival-img-wrap">
                ...
                <div class="ts-arrival-actions">
                  ...
                  <button class="ts-icon-btn" title="Xóa" @click.stop.prevent="onDelete(product)">🗑️</button>
                </div>
              </div>
...
const heroSlides = [
  {
    type: "video",
    src: mainVideo,
    badge: "Quản trị hệ thống · 2026",
    titleLines: [
      { text: "NhutAnhStore", cls: "ts-title-line--dim" },
      { text: "Admin", cls: "ts-title-line--main" },
      { text: "Tổng Quan", cls: "ts-title-line--accent" },
    ],
    sub: "Quản lý sản phẩm · Đơn hàng · Khách hàng · Doanh thu",
    cta: "Quản lý sản phẩm",
  },
  {
    type: "image",
    src: heroImage,
    badge: "NhutAnhStore · Bảng điều khiển",
    titleLines: [
      { text: "TOÀN CẢNH", cls: "ts-title-line--dim" },
      { text: "CỬA HÀNG", cls: "ts-title-line--main" },
    ],
    sub: "Theo dõi tồn kho, đơn hàng và đánh giá mới nhất",
    cta: "Xem thống kê",
  },
  {
    type: "promo",
    badge: "⚡ Flash Sale đang chạy",
    titleLines: [
      { text: "Đang giảm đến", cls: "ts-title-line--dim" },
      { text: "50%", cls: "ts-title-line--main ts-title-line--fire" },
    ],
    sub: "Kiểm tra và điều chỉnh chương trình khuyến mãi",
    cta: "Quản lý Flash Sale",
  },
];
...
async function onDelete(product) {
  ...
  try {
    ...
    newArrivals.value = newArrivals.value.filter(p => p._id !== product._id);
...
onMounted(async () => {
  try {
    const res = await productService.getAll({ limit: 20, sortBy: "sold", sortOrder: "desc" });
    const products = (res.products || res).slice(0, 20);
...
/* ═══ ADMIN BADGE (fixed) ═══ */
.ts-admin-badge-wrap { position: fixed; top: 20px; right: 24px; z-index: 9999; }
.ts-admin-badge {
  display: inline-flex; align-items: center; gap: 8px;
...
.ts-badge-dot { width: 8px; height: 8px; background: #ef4444; border-radius: 50%; animation: blink 2s ease-in-out infinite; }
...
.ts-title-line--accent {
  ...
  background: linear-gradient(135deg, #ef4444, #f97316, var(--ts-accent-3));
  -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text;
}
.ts-title-line--fire {
...
.ts-hero-actions { display: flex; gap: 16px; justify-content: center; flex-wrap: wrap; }
...
/* ════════════════════════════════════════
   QUICK NAV
════════════════════════════════════════ */
.ts-quicknav { background: var(--ts-surface); border-bottom: 1px solid var(--ts-border); padding: 48px 0; }
.ts-quicknav-label { font-size: .7rem; letter-spacing: .2em; color: var(--ts-text-dim); margin-bottom: 24px; text-align: center; }
.ts-quicknav-grid { display: flex; flex-wrap: wrap; justify-content: center; gap: 24px; }
.ts-nav-item { display: flex; flex-direction: column; align-items: center; gap: 10px; font-size: .85rem; color: var(--ts-text-dim); cursor: pointer; text-decoration: none; transition: color .2s; }
.ts-nav-item:hover { color: var(--ts-text); }
.ts-nav-icon-wrap {
  position: relative; width: 72px; height: 72px; border-radius: 18px;
  background: var(--ts-surface-2); border: 1px solid var(--ts-border); overflow: hidden;
  display: flex; align-items: center; justify-content: center; transition: border-color .2s;
}
.ts-nav-item:hover .ts-nav-icon-wrap { border-color: var(--ts-accent); }
.ts-nav-icon { font-size: 1.8rem; }
.ts-nav-glow { position: absolute; inset: 0; background: radial-gradient(circle, rgba(0,212,255,.18), transparent 70%); opacity: 0; transition: opacity .2s; }
.ts-nav-item:hover .ts-nav-glow { opacity: 1; }

/* ════════════════════════════════════════
   CATEGORIES
════════════════════════════════════════ */
.ts-cats { background: var(--ts-bg); border-bottom: 1px solid var(--ts-border); padding: 44px 0; }
...
.ts-hot-inner:hover { border-color: rgba(239,68,68,0.4); transform: translateY(-5px); box-shadow: 0 20px 40px rgba(0,0,0,0.4), 0 0 40px rgba(239,68,68,.1); }
...
.ts-btn-view {
  background: #ef4444; color: #fff; border: none; padding: 10px 20px; border-radius: 999px;
...
.ts-flash-admin-actions { display: flex; }
.ts-btn-cart {
  width: 100%; padding: 10px; border: none; background: linear-gradient(135deg, var(--ts-gold), #f59e0b);
  color: #000; border-radius: 10px; font-weight: 700; cursor: pointer; font-size: 0.85rem;
  text-align: center; text-decoration: none; display: block; transition: all .25s;
}
...
/* ════════════════════════════════════════
   REVIEWS CAROUSEL
════════════════════════════════════════ */
.ts-reviews { padding: 88px 0; background: var(--ts-surface); border-top: 1px solid var(--ts-border); }
...
.ts-review-card { background: var(--ts-bg); border: 1px solid var(--ts-border); border-radius: var(--ts-radius-lg); padding: 32px; }
...
/* ════════════════════════════════════════
   NEW ARRIVALS
════════════════════════════════════════ */
.ts-arrivals { padding: 88px 0; background: var(--ts-bg); border-top: 1px solid var(--ts-border); }
.ts-arrivals-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(200px,1fr)); gap: 18px; }
.ts-arrival-card { background: var(--ts-surface); border: 1px solid var(--ts-border); border-radius: var(--ts-radius-lg); overflow: hidden; transition: all .3s; }
.ts-arrival-card:hover { border-color: rgba(16,185,129,0.4); transform: translateY(-5px); box-shadow: 0 16px 40px rgba(0,0,0,0.3), 0 0 30px rgba(16,185,129,.08); }
...
.ts-icon-btn {
  width: 34px; height: 34px; background: rgba(13,17,23,0.85); backdrop-filter: blur(8px);
  border: 1px solid var(--ts-border); border-radius: 50%; cursor: pointer; font-size: 0.85rem;
  display: flex; align-items: center; justify-content: center; transition: border-color .2s; text-decoration: none;
}
...
.ts-stock-dot { width: 7px; height: 7px; border-radius: 50%; background: var(--ts-green); box-shadow: 0 0 6px var(--ts-green); animation: blink 2s ease-in-out infinite; flex-shrink: 0; }
...
@media (max-width: 768px) {
  ...
  .ts-admin-badge-wrap { top: 12px; right: 12px; }
}
</style>