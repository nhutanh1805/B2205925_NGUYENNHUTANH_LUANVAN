<template>
  <div
    class="products-page min-h-screen bg-gradient-to-b from-blue-50 to-white py-10"
  >
    <div class="container mx-auto max-w-7xl px-4 relative">
      <div
        class="hero-box relative overflow-hidden text-white mb-10 rounded-3xl shadow-2xl"
      >
        ...
        <div class="relative z-10 py-14 px-6 text-center">
          ...
          <p class="text-xl opacity-90 mb-8">
            Ốp lưng • Cáp sạc • Tai nghe • Sạc nhanh • Chính hãng • Giá tốt
          </p>
      ...
      <div class="bg-white rounded-3xl shadow-xl p-6 sticky top-6 z-20">
        ...
        <div v-if="filteredProductsCount" class="product-scroll">
          ...
          >
            ...
            <span v-else-if="product.stock === 0" class="badge badge-out"
              >HẾT HÀNG</span
            >
            ...
            <h4 class="mini-name" @click="goDetail(product._id)">
              {{ product.name }}
            </h4>
            ...
            <button
              ...
              class="add-to-cart-btn w-full mt-3 py-3 rounded-xl font-bold text-black shadow-lg flex items-center justify-center gap-2"
              :class="{
        ...
        <p v-else class="text-center text-gray-500 italic py-12 text-xl">
      ...
      <transition name="modal">
        ...
        >
          <div
            class="bg-white dark:bg-gray-800 rounded-3xl shadow-3xl p-10 max-w-md w-full mx-4 transform transition-all scale-100"
          >
            <div class="text-center">
              ...
              <div class="flex gap-6 justify-center">
                <button
                  @click="cancelAdd"
                  class="px-10 py-4 bg-gray-400 hover:bg-gray-500 text-black font-bold rounded-2xl transition-all hover:scale-105"
                >
                  Hủy
                </button>
                <button
                  @click="executeAddToCart"
                  class="px-12 py-4 bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-black font-bold rounded-2xl transition-all hover:scale-105 shadow-xl"
                >
                  Có, thêm ngay!
                </button>
              </div>
      ...
      <transition name="toast">
        <div v-if="showToast" class="fixed top-6 right-6 z-50">
          <div
            class="bg-gray-900 text-white px-8 py-5 rounded-2xl shadow-2xl flex items-center gap-5 animate-fade-in"
          >
            <i class="fas fa-check-circle text-green-400 text-3xl"></i>
            <div>
              <p class="font-bold text-lg">localhost thông báo:</p>
              <p class="text-base opacity-90">
                Đã thêm "{{ toastMessage }}" vào giỏ hàng
              </p>
            </div>
          </div>
        </div>
...
.hero-bg {
  ...
  background:
    ...
    radial-gradient(circle at 50% 100%, rgba(0, 0, 0, 0.2), transparent 50%);
  filter: blur(20px);
}
.hero-stats {
  display: flex;
